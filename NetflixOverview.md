# Netflix SQL Analysis Project


## Overview

This project analyzes Netflix's catalog through a dataset using SQL (SQLite).  

Conducted an analysis of Netflix's content library to extract business insights across content type, ratings, top directors/countries, and content growth trends over the years.



The dataset contains \*\*8,807 titles\*\* including movies and TV shows.



---



## Dataset Information



- Source: Kaggle (Netflix Dataset)

- Total Records: 8,807

- Columns:

  - show_id

  - type (Movie / T.V. Show)

  - title

  - director

  - cast

  - country

  - date_added

  - release_year

  - rating

  - duration

  - listed_in

  - description



---



## Tools Used



- SQLite (DB Browser for SQLite)

- SQL (Data Analysis)

- CSV Dataset



---



## Objectives



The main objectives of this project were:



- Study Netflix content growth over time

- Discover release year trends

- Analyze content ratings

- Understand content distribution 

- Identify top directors

- Identify top contributing countries



---



## Key SQL Analyses Performed



### 1. Content Type Distribution

- Movies: 6131  

- TV Shows: 2676  



---



### 2. Content Ratings Analysis

- Most common rating: TV-MA (3207)

- Followed by: TV-14, TV-PG



---



### 3. Top 10 Countries

- United States dominates with 2818 titles

- India is second with 972 titles



---



### 4. Release Year Trends

- Peak content years: 2017–2019

- Maximum titles in 2018 (1147)



---



### 5. Top Directors

- Rajiv Chilaka (19 titles)

- Raúl Campos \& Jan Suter (18 titles)



---



### 6. Content Growth Over Time

- Major expansion after 2016

- Peak additions in 2019 (2016 titles)



---




## Key Insights



- Netflix content library is heavily dominated by movies

- USA contributes the largest share of content

- Netflix expanded rapidly between 2016–2019

- T.V. shows are generally newer than movies

- The top directors contributed multiple titles ranging from 11 to 19



---



## How to Run This Project



1. Open DB Browser for SQLite

2. Load `NetflixTitles.csv`

3. Run queries from `NetflixQueries.sql`

4. Explore results and insights



---



## Author



Matthew Wong  

B.A Business Administration, Information Systems & Decision Science  

Aspiring Data Analyst
